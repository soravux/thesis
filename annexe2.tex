...
As $\left( \mathbf{l}_i^T\right)^2$ is the result of the outer product $\mathbf{l}_i^T \mathbf{l}_i$, is it positive semidefinite~\cite{schwerdtfeger1950introduction}. Since the second derivative (Hessian) of $f_i(\mathbf{n})$ is either null or a positive semidefinite matrix, $f_i(\mathbf{n})$ is hence convex~\cite{schwerdtfeger1950introduction}. The objective function $\sum_{i \in \mathcal{I}} f_i(\mathbf{n})$ is then the sum of convex functions, which gives also a convex function~\cite{schwerdtfeger1950introduction}.
